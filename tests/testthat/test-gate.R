...
test_that("sw_build_gating_template creates myeloid template", {
  tmp_file <- tempfile(fileext = ".csv")
  on.exit(unlink(tmp_file))

  sw_build_gating_template(tmp_file, template_type = "myeloid")
  gt <- utils::read.csv(tmp_file, stringsAsFactors = FALSE)
...
test_that("sw_build_gating_template creates nk template", {
  tmp_file <- tempfile(fileext = ".csv")
  on.exit(unlink(tmp_file))

  sw_build_gating_template(tmp_file, template_type = "nk")
  gt <- utils::read.csv(tmp_file, stringsAsFactors = FALSE)
  expect_true(nrow(gt) >= 4)
...
test_that("sw_build_gating_template creates treg template", {
  tmp_file <- tempfile(fileext = ".csv")
  on.exit(unlink(tmp_file))

  sw_build_gating_template(tmp_file, template_type = "treg")
  gt <- utils::read.csv(tmp_file, stringsAsFactors = FALSE)
...
test_that("sw_build_gating_template creates full_pbmc template", {
  tmp_file <- tempfile(fileext = ".csv")
  on.exit(unlink(tmp_file))

  sw_build_gating_template(tmp_file, template_type = "full_pbmc")
  gt <- utils::read.csv(tmp_file, stringsAsFactors = FALSE)
...
test_that("sw_gate runs end-to-end with synthetic FCS", {
  skip_if_not_installed("flowCore")
  skip_if_not_installed("flowWorkspace")
  ...
  sw_build_gating_template(tmp_template)

  gs <- sw_gate(tmp_fcs, tmp_template)
...
test_that("sw_extract_gated returns named list of flowFrames", {
  skip_if_not_installed("flowCore")
  skip_if_not_installed("flowWorkspace")
  skip_if_not_installed("openCyto")
  ...
  ff_list <- sw_extract_gated(gs, leaf)
