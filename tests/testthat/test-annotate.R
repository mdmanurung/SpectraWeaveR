...
test_that("sw_annotate_load_ref returns a list with matrix and mask", {
...
test_that("sw_annotate_load_ref matrix and mask have identical dimensions", {
  ref <- sw_annotate_load_ref("pbmc")
  expect_identical(dim(ref$matrix), dim(ref$mask))
  expect_identical(dimnames(ref$matrix), dimnames(ref$mask))
})

test_that("sw_annotate_load_ref reference has expected populations", {
  ref <- sw_annotate_load_ref("pbmc")
...
test_that("sw_annotate_load_ref matrix values are in 0-3 range", {
  ref <- sw_annotate_load_ref("pbmc")
  expect_true(all(ref$matrix >= 0))
  expect_true(all(ref$matrix <= 3))
})

test_that("sw_annotate_load_ref mask values are binary 0/1", {
  ref <- sw_annotate_load_ref("pbmc")
  expect_true(all(ref$mask %in% c(0L, 1L)))
})

test_that("sw_annotate_load_ref rejects unsupported name", {
  expect_error(sw_annotate_load_ref("t_cell"), "should be one of")
})

test_that("sw_annotate_load_ref rejects non-character name", {
  expect_error(sw_annotate_load_ref(1L), "single non-empty character")
})

# ---------------------------------------------------------------------------
# .check_annotation_input (via sw_annotate_run)
# ---------------------------------------------------------------------------

test_that("sw_annotate_run rejects non-data.frame non-cluster_result", {
  expect_error(sw_annotate_run("not_valid"), "sw_cluster_result")
})

test_that("sw_annotate_run rejects data.frame missing cluster column", {
  df <- data.frame(CD3 = 1, CD4 = 2)
  expect_error(sw_annotate_run(df), "'cluster' column")
})

test_that("sw_annotate_run rejects data.frame with no numeric markers", {
  df <- data.frame(cluster = 1L, label = "A", stringsAsFactors = FALSE)
  expect_error(sw_annotate_run(df), "numeric marker")
})

test_that("sw_annotate_run rejects invalid reference structure", {
  mfi <- .make_mfi(2)
  expect_error(
    sw_annotate_run(mfi, reference = list(not_matrix = 1)),
    "'matrix' and 'mask'"
  )
})

test_that("sw_annotate_run rejects non-matrix reference$matrix", {
  mfi <- .make_mfi(2)
  ref <- list(matrix = data.frame(CD3 = 1), mask = matrix(1L, 1, 1))
  expect_error(sw_annotate_run(mfi, reference = ref), "must be matrices")
})

test_that("sw_annotate_run rejects min_score outside [0,1]", {
  mfi <- .make_mfi(2)
  expect_error(sw_annotate_run(mfi, min_score = -0.1), "\\[0, 1\\]")
  expect_error(sw_annotate_run(mfi, min_score = 1.5),  "\\[0, 1\\]")
})

test_that("sw_annotate_run rejects markers not in MFI data", {
  mfi <- .make_mfi(2)
  expect_error(
    sw_annotate_run(mfi, markers = c("CD3", "NOT_A_MARKER")),
    "not found"
  )
})

test_that("sw_annotate_run rejects too few overlapping markers", {
  ...
  expect_error(sw_annotate_run(mfi, reference = ref),
               "Fewer than 2 markers")
})

# ---------------------------------------------------------------------------
# sw_annotate_run — correctness
# ---------------------------------------------------------------------------

test_that("sw_annotate_run returns tibble with expected columns", {
  mfi    <- .make_mfi(5)
  result <- sw_annotate_run(mfi)
...
test_that("sw_annotate_run assigns CD4-related type to a CD4+ cluster", {
  mfi    <- .make_mfi(1)  # first row: CD3+ CD4+ TCRab+ pattern
  result <- sw_annotate_run(mfi, min_score = 0)
...
test_that("sw_annotate_run assigns B-cell type to a B-cell cluster", {
  mfi <- tibble::tibble(
    cluster = 1L,
    CD3 = 0, CD4 = 0, CD8a = 0,
...
test_that("sw_annotate_run sets NA when score below min_score", {
  mfi <- tibble::tibble(
    cluster = 1L,
    CD3 = 1.5, CD4 = 1.5, CD19 = 1.5, CD56 = 1.5, CD14 = 1.5
  )
  # Using a very high min_score should force NA
  result <- sw_annotate_run(mfi, min_score = 0.9999)
  expect_true(is.na(result$cell_type[1L]))
})

test_that("sw_annotate_run respects user-supplied markers subset", {
  mfi    <- .make_mfi(3)
  result <- sw_annotate_run(mfi, markers = c("CD3", "CD4", "CD8a",
                                                    "CD19", "CD20"))
  expect_equal(result$n_markers_used[1L], 5L)
})

test_that("sw_annotate_run works with a custom reference", {
  mfi <- tibble::tibble(
    cluster = 1L,
    MarkerA = 3,
    MarkerB = 0,
  ...
  mask <- matrix(c(1L, 1L, 1L, 1L, 1L, 1L), nrow = 2,
                 dimnames = list(c("Pop1", "Pop2"),
                                 c("MarkerA", "MarkerB", "MarkerC")))
  ref    <- list(matrix = mat, mask = mask)
  result <- sw_annotate_run(mfi, reference = ref, min_score = 0)
  expect_equal(result$cell_type[1L], "Pop1")
})

test_that("sw_annotate_run score is in [0, 1]", {
  mfi    <- .make_mfi(5)
  result <- sw_annotate_run(mfi, min_score = 0)
...
test_that("sw_annotate_run score >= second_score", {
  mfi    <- .make_mfi(5)
  result <- sw_annotate_run(mfi, min_score = 0)
